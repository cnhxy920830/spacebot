...
fn read_filesystem_usage(path: &Path) -> anyhow::Result<StorageStatus> {
    let (total_bytes, available_bytes) = read_filesystem_capacity(path)?;
...
#[cfg(unix)]
fn read_filesystem_capacity(path: &Path) -> anyhow::Result<(u64, u64)> {
    ...
    Ok(((block_size * total_blocks) as u64, (block_size * available_blocks) as u64))
}
...
fn read_filesystem_capacity(_path: &Path) -> anyhow::Result<(u64, u64)> {
    Err(anyhow::anyhow!(
        "storage status is not supported on this platform"
    ))
}

...
# Optional OpenAI-compatible base URL (e.g. Ark):
...
workspace_allowlist = ["/extra/project/path", "/mnt/shared-data"] # optional additional roots for worker file/shell/exec access
...
| NVIDIA NIM | `nvidia/<model>` | `nvidia/meta/llama-3.1-70b-instruct` |
...
| `openai_base_url` | string | None | Optional OpenAI-compatible base URL. If set, Spacebot calls `{base_url}/chat/completions`. |
| `nvidia_key` | string | None | NVIDIA API key for `nvidia/<model>` routing (or `env:VAR_NAME`) |
